...
class MetaOxmlElement(type):
    ...
    def create(cls):
        """
        cls.__dict__ sometimes contains '__dict__' inside it.
        That __dict__ proxy is then improperly copied to the new_class by type()
        which results in the new_class having a broken cls().__dict__ attribute.
        Attempting to access an instance's __dict__ attribute (new_class().__dict__)
...
class BaseOxmlElement(etree.ElementBase, metaclass=MetaOxmlElement):
    ...
    def xpath(self, xpath_str: str, namespaces: dict[str, str] | None = None) -> Any:  # pyright: ignore[reportIncompatibleMethodOverride]
        """Override of `lxml` _Element.xpath() method.

        Provides standard Open XML namespace mapping (`nsmap`) in centralized location
        if `namespaces` is not provided.
        """
        return super().xpath(xpath_str, namespaces=namespaces or nsmap)
