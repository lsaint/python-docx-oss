...
class CT_RPr(BaseOxmlElement):
    ...
    eastAsianLayout: CT_OnOff | None = ZeroOrOne("w:eastAsianLayout", successors=_tag_seq[37:])
    ...
    def rFonts_ascii(self, value: str | None) -> None:
        if value is None:
            if self.rFonts is not None:
                self.rFonts.ascii = None
            return
    ...
    def rFonts_eastAsia(self, value: str | None) -> None:
        if value is None:
            if self.rFonts is not None:
                self.rFonts.eastAsia = None
            return
        rFonts = self.get_or_add_rFonts()
