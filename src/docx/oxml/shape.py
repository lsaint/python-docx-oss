...
if TYPE_CHECKING:
    ...
    from docx.oxml.dml.CT_ExtensionList import CT_ExtensionList
...
class CT_Anchor(BaseOxmlElement):
    ...
    def new(
        ...
        as parameters.
        """
        anchor = cast(CT_Anchor, parse_xml(cls._anchor_xml(pos_x, pos_y)))
        anchor.extent.cx = cx
        anchor.extent.cy = cy
    ...
    @classmethod
    def _anchor_xml(cls, pos_x: int | Length, pos_y: int | Length):
        return (
...
class CT_Blip(BaseOxmlElement):
    ...
    extLst: CT_ExtensionList | None = ZeroOrOne("a:extLst")
...
class CT_Picture(BaseOxmlElement):
    ...
    def new(cls, pic_id: int, filename: str, rId: str, cx: Length, cy: Length) -> CT_Picture:
        ...
        if filename.endswith(".svg"):
            pic = cast(CT_Picture, parse_xml(cls._pic_xml_svg()))
            blip = pic.blipFill.blip
            assert blip is not None
            blip.extLst.ext.svgBlip.embed = rId
        else:
