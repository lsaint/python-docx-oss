...
class DescribeFont:
    ...
    @pytest.mark.parametrize(
        ("r_cxml", "expected_ascii", "expected_hAnsi", "expected_eastAsia"),
        [
            ("w:r", None, None, None),
            ("w:r/w:rPr", None, None, None),
            ("w:r/w:rPr/w:rFonts", None, None, None),
            ("w:r/w:rPr/w:rFonts{w:ascii=Arial}", "Arial", None, None),
            ("w:r/w:rPr/w:rFonts{w:hAnsi=Arial}", None, "Arial", None),
            ("w:r/w:rPr/w:rFonts{w:eastAsia=Arial}", None, None, "Arial"),
        ],
    )
    def it_knows_its_typeface_name(
        self, r_cxml: str, expected_ascii: str | None, expected_hAnsi: str | None, expected_eastAsia: str | None
    ):
        r = cast(CT_R, element(r_cxml))
        font = Font(r)
        assert font.name == expected_ascii
        assert font.hAnsi == expected_hAnsi
        assert font.eastAsia == expected_eastAsia
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "w:r",
                {"ascii": "Arial", "hAnsi": "Times New Roman", "eastAsia": "Microsoft YaHei"},
                "w:r/w:rPr/w:rFonts{w:ascii=Arial,w:hAnsi='Times New Roman',w:eastAsia='Microsoft YaHei'}",
            ),
            (
                "w:r/w:rPr/w:rFonts{w:ascii=Foo}",
                {"hAnsi": "Bar", "eastAsia": "Baz"},
                "w:r/w:rPr/w:rFonts{w:ascii=Foo,w:hAnsi=Bar,w:eastAsia=Baz}",
            ),
        ],
    )
    def it_can_change_its_typeface_name(self, r_cxml: str, value: str | dict[str, str | None], expected_r_cxml: str):
        r = cast(CT_R, element(r_cxml))
        font = Font(r)
