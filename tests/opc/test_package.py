...
class DescribeOpcPackage:
    ...
    def it_can_drop_relationships_by_cmp_function(self, rels_prop_: Mock, rels_: Mock):
        """Test drop_rels method."""
        # mockery ----------------------
        pkg = OpcPackage()
        rels_prop_.return_value = rels_

        # create mock relationships
        rel1 = Mock(name="rel1", rId="rId1", is_external=True)
        rel2 = Mock(name="rel2", rId="rId2", is_external=False)
        rel3 = Mock(name="rel3", rId="rId3", is_external=True)

        rels_.items.return_value = [("rId1", rel1), ("rId2", rel2), ("rId3", rel3)]

        # define cmp function that matches external relationships
        def is_external(rel):
            return rel.is_external

        # exercise ---------------------
        dropped_rids = pkg.drop_rels(is_external)

        # verify -----------------------
        assert dropped_rids == ["rId1", "rId3"]
        rels_.__delitem__.assert_any_call("rId1")
        rels_.__delitem__.assert_any_call("rId3")
        assert rels_.__delitem__.call_count == 2
